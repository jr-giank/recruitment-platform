...
const FiltersWithCheckBox = ({itemsList, setFilters, filters, target}) => {
    ...
    <div className='flex flex-col'>
        {
            itemsList.map(item => (
                <div key={item} className='flex items-center px-3'>
                    <input type="checkbox" name={item} id={`${item}`} value={item} className='checkbox_filter' onChange={handleOnCheck} />
